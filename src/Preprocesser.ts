...
class Preprocesser {
  ...
  paragraphToSentences(string_to_process: any) {
    try {
  ...
  cleanSentences(list_to_clean: any[]) {
  ...
  tokenizeSentences(list_of_sentences: any[]) {
  ...
  getFrequencyAndMax(list_of_words: string | any[]) {
  ...
  getWeights(list_of_words: any) {
    const frequencies_and_max = this.getFrequencyAndMax(list_of_words);
    const frequencies_map: any = frequencies_and_max[0];
    const max: any = frequencies_and_max[1];
    frequencies_map.forEach(
      (
        value: number,
        key: any,
        map: { set: (arg0: any, arg1: number) => void }
      ) => {
  ...
  sentenceWeights(
    clean_sentences: string | any[],
    weighted_map: { get: (arg0: string) => number }
  ) {
  ...
  async nounsAndAdjectives(clean_sentences: string | any[]) {
  ...
  getEdgeWeights(list1: string | any[], list2: string | any[]) {
    let weight = 0;
    let intial = list1;
    let other = list2;
  ...
  createTextRankGraph(nouns_and_adjactive_map: {
    forEach: (arg0: (value: any, key: any, map: any) => void) => void;
    get: (arg0: any) => any;
  }) {
    let graph = new WeightedGraph();
    let key_list: any[] = [];
    let weight = 0;
    nouns_and_adjactive_map.forEach((value: any, key: any, map: any) => {
      key_list.push(key);
    });
    for (let i = 0; i < key_list.length; i++) {
      for (let j = i + 1; j < key_list.length; j++) {
        weight = this.getEdgeWeights(
          nouns_and_adjactive_map.get(key_list[i]),
  ...
  textRank(graph: {
    ...
    let probability_list: any[] = [];
    //random walk
    for (let i = 0; i < 10000; i++) {
      let full_weight = 0;

      vertex.adjacent.forEach((value: number, key: any, map: any) => {
        full_weight += value;
      });
